...
### IMPORT MODULES
Import-Module .\Modules\DriverPack -Force -Global -ErrorAction Stop -Verbose
...
function Main {
    ...
    begin {
        ...
        if ($null -eq $DeviceModel) {
            ...
            #Filter
            $DeviceModel = Import-DeviceModels | Out-GridView -OutputMode Single -Title "Select Device Model"
            if ($null -eq $DeviceModel) { break }
        }
        
        #Filter
        $Catalog = Get-DriverPackCatalog | Where-Object { 
            $_.Manufacturer -eq $DeviceModel.Manufacturer -and 
            $_.Model -eq $DeviceModel.Model -and 
            $_.Types -match $DeviceModel.Type 
        }

        $ItemCatalog = $Catalog | Out-GridView -OutputMode Single -Title "Select driverpack from catalog"
        if ($null -eq $ItemCatalog) { break }

    }
    process {
        <##>
        $newDriverpack = Add-DriverPack -DeviceModelGuid $DeviceModel.Guid -InputObject $ItemCatalog

    }
    end {
        <##>
        Write-Host "New driverpack add for $($DeviceModel.Model)." -ForegroundColor Green
        return $newDriverpack
    }
}
Main
