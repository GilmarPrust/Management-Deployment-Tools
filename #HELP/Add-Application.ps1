...
+----------------------------------------------------------------------------------------+
    .NOTES
        ...
        Filename: Add-Application.ps1
    .DESCRIPTION
        Adiciona um aplicativo em Applications.json

    .EXAMPLE
        $app = @{
            
            NameID      = "adobrereaderdc_12.3.4"    | Sugestão de uso: DisplayName + _ + Version
            DisplayName = "Adobe Reader DC MUI"
            Version     = "12.3.4"
            FileName    = "setup.exe"
            Argument    = "/silent /norestart"
            Source      = "\Applications\$NameID"
        }
...
### IMPORT MODULES
Import-Module .\Modules\Application -Force -ErrorAction Stop -Verbose
...
function Main {
    ...
    process {
        <##>
        #Get-Applications
        #Add-Application -NameID $app.NameID -DisplayName $app.DisplayName -Version $app.Version -FileName $app.FileName -Argument $app.Argument -Source $app.Source  
        Add-Application @app
    }
    end {
        <##>
