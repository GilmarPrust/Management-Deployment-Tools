...
{
    public class DriverPackService : IDriverPackService
    {
        private readonly AppDbContext _context;
        private readonly IMapper _mapper;
        private readonly ILogger<DriverPackService> _logger;

        public DriverPackService(AppDbContext context, IMapper mapper, ILogger<DriverPackService> logger)
        {
            _context = context;
            _mapper = mapper;
            _logger = logger;
        ...
        public async Task<IEnumerable<DriverPackReadDTO>> GetAllAsync()
        {
            try
            {
        ...
        public async Task<DriverPackReadDTO?> GetByIdAsync(Guid id)
        {
            if (id == Guid.Empty)
        ...
        public async Task<DriverPackReadDTO> CreateAsync(DriverPackCreateDTO dto)
        {
            ...
            try
            {
                ...
                return _mapper.Map<DriverPackReadDTO>(entity);
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> UpdateAsync(Guid id, DriverPackUpdateDTO dto)
        {
            if (id == Guid.Empty)
                throw new ArgumentException("Id não pode ser vazio.", nameof(id));
            if (dto == null)
