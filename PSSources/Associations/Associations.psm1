...
function Get-ProfileApplications {
    ...
    $devices = Import-DeviceApplications

    $devices = $devices | Where-Object { $_.DeviceGuid -eq $DeviceGuid }
...
function Get-DeviceModelApplications {
    <#
        .DESCRIPTION
    #>
    [CmdletBinding()]
    ...
    $devices = Import-DeviceModelApplications

    $devices = $devices | Where-Object { $_.DeviceModelGuid -eq $DeviceModelGuid }
...
function Get-DeviceProfile {
    ...
    ###
    Install-PSResource Control -Repository DCM2_PSResources
    Install-PSResource Profiles -Repository DCM2_PSResources

    $deviceprofile = Import-DeviceProfile | Where-Object { $_.DeviceGuid -contains $DeviceGuid }

    if ($null -eq $deviceprofile) { 

        $guid = (Get-Content "$($global:DCM2Root)\Settings.json" | ConvertFrom-Json).DefaultProfile
        return Get-Profile -Guid $guid
        #Set-DeviceProfile
    }

    return Get-Profile -Guid $deviceprofile.ProfileGuid
}
