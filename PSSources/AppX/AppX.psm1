...
function Remove-ProvisionedAppx {
    ...
    $percentItem = 100/$List.Count
    $percentRemoved = 0

    foreach ($item in $List)
    {
        Write-Progress -Id 1 -Activity "Remove Provisioned Appx" -Status " $($item)" -PercentComplete $percentRemoved
        
        switch ($Online.IsPresent) {

            $false { 
                $package = Get-AppxProvisionedPackage -Path "$($global:DriveSystem)\" | Where-Object { $_.Displayname -eq $item }
                if ($null -eq $Package) { continue }
                Remove-AppxProvisionedPackage -Path "$($global:DriveSystem)\" -PackageName $package.PackageName | Out-Null
            }
            $true { 
                $package = Get-AppxProvisionedPackage -Online | Where-Object { $_.Displayname -eq $item }
                if ($null -eq $Package) { continue }
                Remove-AppxProvisionedPackage -Online -PackageName $package.PackageName | Out-Null
            }
        }
        Write-Host "$($item) removed." -ForegroundColor DarkGray
