...
function Build-Unattend {
    ...
    # Esta função nao deve acessar vasiaveis gloval, apenas recebe parametros executa e retorna. 
    # vai receber varios paramentros, criar unattend, depois retornar arquivo criado

    Write-Host "  Building Unattend.xml file." -ForegroundColor Yellow

    [xml]$XmlUnattend = Get-Content "$($PSScriptRoot)\Unattend.xml" -ErrorAction Stop

    ### Update Manufacturer on XML.
    (($XmlUnattend.unattend.settings | 
        Where-Object {$_.pass -eq 'specialize'}).component | 
        Where-Object {$_.name -eq 'Microsoft-Windows-Shell-Setup'}).OEMInformation.Manufacturer = $Manufacturer

    ### Update ComputerName on XML.
    (($XmlUnattend.unattend.settings | 
        Where-Object {$_.pass -eq 'specialize'}).component | 
        Where-Object {$_.name -eq 'Microsoft-Windows-Shell-Setup'}).ComputerName = $ComputerName
    
    return [xml]$XmlUnattend
}

function ApplyUnattend {
    <#
        .DESCRIPTION
        Apply on System drive.
        .NOTES
        #Use-WindowsUnattend -Path "$($global:DriveSystem)\" -UnattendPath $unattendfile -NoRestart -Verbose
    #>
    param (
    ...
    $unattendfile = "$($DriveSystem)\Windows\Panther\unattend.xml"
    New-Item -Path "$($DriveSystem)\Windows\" -Name "Panther" -ItemType Directory -Force | Out-Null

    $XmlUnattend.Save($unattendfile)

    ### Apply Unattend.xml on system drive.
    Invoke-Command -ScriptBlock { 

        Push-Location -Path "C:\Windows\System32"
        Set-Location -Path "$($global:DeployRoot)\Tools\amd64\DISM"
        Dism.exe /image:"$($DriveSystem)\" /Apply-Unattend:"$($unattendfile)" | Out-Null
        Pop-Location
    }
}
