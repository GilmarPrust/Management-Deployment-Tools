namespace DCM.API.Endpoints
{
    ...
    public static class ApplicationEndpoints
    {
        ...
        public static RouteGroupBuilder MapApplicationEndpoints(this RouteGroupBuilder group)
        {
            ...
            // POST Application
            group.MapPost("/", async (ApplicationCreateDTO dto, IApplicationService service, IValidator<ApplicationCreateDTO> validator) =>
            {
                if (dto == null)
            ...
            // PUT Application
            group.MapPut("/{id:guid}", async (Guid id, ApplicationUpdateDTO dto, IApplicationService service, IValidator<ApplicationUpdateDTO> validator) =>
            {
                if (dto == null)
                    return Results.BadRequest("Dados obrigatórios não informados.");
                var validation = await validator.ValidateAsync(dto);
