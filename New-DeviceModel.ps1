...
function Main {
    ...
    begin {
        ...
        if ($Manufacturer -ne "") {
            $catalog = $catalog | Where-Object { $_.Manufacturer -eq $Manufacturer }
        }
        if ($Model -ne "") {
            $catalog = $catalog | Where-Object { $_.Model -eq $Model }
        }
        if ($Type -ne "") {
            $catalog = $catalog | Where-Object { $_.Types -match $Type  }
        }        
    }
    process {
        <#
        ...
        #Cria novo objeto do item selecionado do Catalogo.
        $Script:NewDeviceModel = [PScustomobject]@{ 
                Guid = (New-Guid).Guid;
                Manufacturer="$($SelectedItemCatalog.Manufacturer)";
                Model="$($SelectedItemCatalog.Model)";
                Types=@($SelectedItemCatalog.Types -split ',');
        }
    }
    end {
        <#
        ...
        #>
        $existingdevice = $JsonDeviceModels | Where-Object { ($_.Model -eq $SelectedItemCatalog.Model) -and ($_.Types -match ($SelectedItemCatalog.Types -split ',')[0]) }
        if($null -eq $existingdevice) {

            $Script:JsonDeviceModels += $NewDeviceModel
            Write-Host "New device model: $($NewDeviceModel.Model)" -ForegroundColor Green
            #Update Json
            $Script:JsonDeviceModels | ConvertTo-Json | Set-Content "$($DeployRoot)\Control\DeviceModels.json"
            
        } else {
            Write-Host "Model: $($NewDeviceModel.Model) alread exist." -ForegroundColor Green
            break
        }
    }
}
...
.\Add-Firmware.ps1 -Device $NewDeviceModel -DeviceCatalog $SelectedItemCatalog
.\Add-DriverPack.ps1 -DeviceModel $NewDeviceModel
