...
function Download {
    ...
    Begin {
        ...
        <# PROXY #>
        if ($Global:JsonSettings.Proxy.Enabled -eq $true) {
    ...
    End {
        ...
        if (![string]::IsNullOrEmpty($Hash) ) {
            return CheckFileHash -File $Destination -Hash $Hash -ErrorAction SilentlyContinue
        }
    }
}
...
function CheckFileHash {
    ...
    Write-Host "Getting file Hash... " -ForegroundColor Yellow -NoNewline
    ...
    if ($fileHash -eq $Hash) { 
        Write-Host "Valid file hash: $($fileHash)" -ForegroundColor Green
        return $true 
    } else { 
        Write-Host "Invalid file hash: $($fileHash)" -ForegroundColor Green
        return $false 
    }
}
